...
class Environment:
    ...
    def dispatch(self, vehicle_id: int, task_id: int) -> bool:
        """API for A-group: assign one task to one vehicle if constraints are feasible."""
        ...
        if vehicle.current_node == task.origin_node:
            ...
            self._complete_task(vehicle, task)
            return True
        ...
        vehicle.assign_task(task.id, route=route, target_node=task.origin_node)
    ...
    def step(self, actions: list[tuple[int, int]] | None = None) -> None:
        self._release_tasks()
        self._expire_tasks()
        ...
        if actions:
            for vehicle_id, task_id in actions:
        ...
        for task_id in list(self.assigned_task_ids):
            ...
            if task.deadline < self.current_time:
                ...
                if task.assigned_vehicle is not None:
                    vehicle = self.vehicles[task.assigned_vehicle]
                    if vehicle.assigned_task == task_id:
    ...
    def _complete_task(self, vehicle: Vehicle, task: Task) -> None:
        start_distance = task.assigned_vehicle_distance
        if start_distance is None:
            start_distance = vehicle.total_distance
        service_distance = vehicle.total_distance - start_distance
        task.mark_completed(self.current_time, service_distance=service_distance)
        ...
        vehicle.total_score += task_score
