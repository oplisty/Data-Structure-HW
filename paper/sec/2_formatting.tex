...
The complete simulation loop can be summarized in three stages:
\begin{enumerate}[leftmargin=*]
    \item task release and vehicle state update;
    \item path/battery feasibility checking and scheduling decision;
    \item movement, service, charging, and statistics update.
\end{enumerate}
...
The simulator supports two types of map data. For controlled experiments, we use randomly generated graph scenarios with different numbers of nodes, vehicles, tasks, and charging stations, so that scheduling strategies can be compared under the same settings. As an extension, the codebase also includes a processed Guangzhou Panyu road-network scenario based on Guangdong Province OSM data. The Panyu road topology is converted into a weighted graph with road points and intersections as nodes, road segments as weighted edges, and charging stations as resource nodes. In the processed metadata, the Panyu map contains 131,276 nodes, 142,593 edges, and 57 charging stations. This allows the same path-planning, scheduling, backend, and frontend pipeline to run on a real urban road network.
...
Beyond the above baselines, we further implement an online reinforcement learning policy selection module based on Q-learning, as well as an offline global optimization module based on MILP under full information. Q-learning is used as a hyper-heuristic selector rather than an end-to-end route generator. Its state includes coarse information such as task backlog, deadline urgency, average battery level, idle vehicle ratio, and charging queue load. Its action is the choice of a low-level dispatching rule such as Nearest, EDF, Heaviest, or a score-based rule. The reward is related to task completion, travel cost, overdue penalty, and charging or waiting cost. These two advanced modules correspond to two different methodological paradigms: online decision-making based on interactive learning and offline solving based on exact optimization, as summarized in Algorithm~\ref{alg:qlearning_policy} and Algorithm~\ref{alg:offline_milp_planning}.

\begin{figure*}[t]
  \centering
  \refstepcounter{algorithm}\label{alg:qlearning_policy}
  \begin{minipage}{0.96\textwidth}
  \textbf{Algorithm \thealgorithm: Q-learning-based Policy Selection}
  \vspace{0.4em}
  \begin{algorithmic}[1]
...
\begin{figure*}[t]
  \centering
  \refstepcounter{algorithm}\label{alg:offline_milp_planning}
  \begin{minipage}{0.96\textwidth}
  \textbf{Algorithm \thealgorithm: Offline MILP-based Global Planning}
  \vspace{0.5em}
